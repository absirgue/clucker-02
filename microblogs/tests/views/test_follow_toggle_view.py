...
class ShowUserTest(TestCase):
    ...
    def setUp(self):
        ...
        self.url = reverse('follow_toggle', kwargs={
                           'user_id': self.followee.id})
    ...
    def test_get_follow_toggle_for_followee(self):
        ...
        followee_followers_before = self.followee.followee_count()
        response = self.client.get(self.url, follow=True)
        user_followers_after = self.user.follower_count()
        followee_followers_after = self.followee.followee_count()
        ...
        response_url = reverse('show_user', kwargs={
                               'user_id': self.followee.id})
    ...
    def test_get_follow_toggle_for_non_followee(self):
        ...
        followee_followers_after = self.followee.followee_count()
        self.assertEqual(user_followers_before, user_followers_after)
        self.assertEqual(followee_followers_before +
                         1, followee_followers_after)
        self.assertEqual(response.status_code, 200)
        response_url = reverse('show_user', kwargs={
                               'user_id': self.followee.id})
        self.assertRedirects(response, response_url,
    ...
    def test_get_follow_toggle_with_invalid_id(self):
        self.client.login(username=self.user.username, password="Password123")
        url = reverse('follow_toggle', kwargs={'user_id': self.user.id+99999})
